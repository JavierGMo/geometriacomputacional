...
def mapNode():
    ...
    countColors = 0
    if numberNodes <= 2:
        raise Exception('El numero de nodos no puede ser menor o igual a 2')
    elif numberNodes == 3:
        ...
        for number in iterNumber:
            ...
            countColors = 0 if (countColors == 2) else incrementCounter(countColors)
...
if __name__ == "__main__":
    ...
    resMap = open('trescolores.txt', 'a')
    resMap.write('Resultado de los tres colores \n')
    resMap.write('Nodos del grafo: \n')
    resMap.write(Gnodes.nodes(data=True).__str__())
    resMap.write('tres colores \n')
    resMap.write('\nRamas: \n')
